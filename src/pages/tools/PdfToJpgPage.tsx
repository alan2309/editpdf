...
export default function PdfToJpgPage() {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: '3rem 1.25rem 6rem' }}>
      <ToolHeader
        title="PDF to JPG"
        description="Convert PDF pages into high-resolution JPG images. Download as individual images or a single ZIP archive."
        icon={<ImageIcon size={24} />}
      />

      {result ? (
        <DownloadResult result={result} onReset={handleReset} toolName="PDF to JPG" />
      ) : isProcessing ? (
          ...
          <div className="card-glass" style={{ padding: '1.5rem', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem' }}>
                {[
                  { val: 72 as const, label: '72 DPI (Web)' },
                  { val: 150 as const, label: '150 DPI (Standard)' },
                  { val: 300 as const, label: '300 DPI (High-Res)' },
                ].map(opt => (
                  <button
                    key={opt.val}
                    type="button"
