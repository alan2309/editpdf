  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: '3rem 1.25rem 6rem' }}>
      <ToolHeader
        ...
        description="Convert PNG images into clean, crisp PDF documents with transparency preservation. 100% private in-browser processing."
        icon={<FileUp size={24} />}
      />

      {result ? (
        <DownloadResult result={result} onReset={handleReset} toolName="PNG to PDF" />
      ) : isProcessing ? (
        ...
        <FileDropzone
          ...
          subtitle="Add one or multiple PNG files to combine into a PDF"
        />
      ) : (
        <div style={{ width: '100%', maxWidth: 680, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
            ...
            >
              <ImageIcon size={18} />
              <span>Create PDF from {files.length} PNG{files.length > 1 ? 's' : ''}</span>
            </button>
