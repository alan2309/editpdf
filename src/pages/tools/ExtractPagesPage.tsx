...
import { FileOutput, FileText, AlertCircle } from 'lucide-react';
...
export default function ExtractPagesPage() {
  ...
  const [pagesStr, setPagesStr] = useState<string>('1');
  ...
  useEffect(() => {
    if (file) {
      getPdfPageCount(file).then(count => {
        setTotalPages(count);
        setPagesStr(count > 1 ? `1-${Math.min(3, count)}` : '1');
      }).catch(() => {
  ...
  return (
    <div style={{ maxWidth: 1080, margin: '0 auto', padding: '3rem 1.25rem 6rem' }}>
      <ToolHeader
        ...
        description="Extract specific pages or page ranges from a PDF into a new, separate PDF file. 100% private in-browser processing."
        icon={<FileOutput size={24} />}
      />

      {result ? (
        <DownloadResult result={result} onReset={handleReset} toolName="Extract Pages" />
      ) : isProcessing ? (
        ...
        <div style={{ width: '100%', maxWidth: 640, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="card-glass" style={{ padding: '1.5rem', borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.08)' }}>
            <PageRangeSelector
              ...
              value={pagesStr}
              onChange={setPagesStr}
              label="Pages to Extract into New PDF:"
            />
          </div>
